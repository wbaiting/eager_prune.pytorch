...
def get_noise(points=10, size=(32, 32, 3)):
    noise = np.zeros(np.prod(size))
    lens = len(noise)
    select_idx = np.random.choice(lens, points, replace=False)
    values = np.random.randint(128, 256, points)
    noise[select_idx] = values
...
def resize_img_folder(in_dir,folder, out_dir, i):
    print(f'{i}Folder {in_dir}')
    ...
    for filename in os.listdir(in_dir):
        ...
        # rain drops
        angle = int(np.random.choice(180, 1))
        point = max(10, int(np.random.normal(100, 25, 1)))
        length = max(20, int(np.random.normal(40, 4, 1)))
        w = max(9, int(np.random.normal(12, 4, 1)))
        if w % 2 == 0:
            w += 1
        blurred = rain_blur(get_noise(points=point, size=np.shape(im)), length=length, angle=angle, w=w)
        im_resized = alpha_rain(blurred, im_resized, beta=0.5)
...
if __name__ == "__main__":
    ...
    for i, folder in enumerate(folders):
        ...
        p.apply_async(resize_img_folder, args=(ifolder, folder, current_out_dir, i))
    p.close()
    p.join()
    print("ok")
